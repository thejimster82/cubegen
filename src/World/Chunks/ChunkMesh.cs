...
public partial class ChunkMesh : Node3D
{
    ...
    private float CalculateAO(VoxelChunk chunk, int x, int y, int z, FaceDirection faceDirection, int vertexIndex)
    {
        ...
        // Special handling for chunk boundaries to ensure consistent AO
        bool isAtBoundary = (x == 0 || x == chunk.Size - 1 || z == 0 || z == chunk.Size - 1);
        if (isAtBoundary)
        {
            // For vertices at chunk boundaries, use a more conservative approach
            // to ensure consistent AO across chunks

            // If we're at a corner of the chunk, reduce the occlusion effect
            if ((x == 0 && z == 0) || (x == 0 && z == chunk.Size - 1) ||
                (x == chunk.Size - 1 && z == 0) || (x == chunk.Size - 1 && z == chunk.Size - 1))
            {
                // At chunk corners, use a fixed medium occlusion value
                return 0.85f;
            }

            // For edges, use a slightly less aggressive occlusion
            if (side1Solid && side2Solid)
            {
                return 0.6f; // Less dark than normal maximum occlusion
            }
            else if (side1Solid || side2Solid)
            {
                return 0.85f; // Less dark than normal medium occlusion
            }
            else
            {
                return 0.95f; // Slightly darker than no occlusion
            }
        }
